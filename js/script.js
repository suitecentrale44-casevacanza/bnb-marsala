...
/* ==========================================
   GOOGLE TRANSLATE E GESTIONE LINGUA FIX
   ========================================== */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Ripristina la bandiera salvata nel browser
  const flagSalvata = localStorage.getItem('flag_selezionata');
  const activeFlag = document.getElementById('activeFlag');
  if (flagSalvata && activeFlag) {
    activeFlag.textContent = flagSalvata;
  }
  ...
  // Timer di sicurezza per lo splash screen
...
function impostaCookieGoogleTranslate(lang) {
  ...
  // Rimuove i cookie precedenti
  document.cookie = "googtrans=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
  document.cookie = "googtrans=; expires=Thu, 01 Jan 1970 00:00:00 UTC; domain=" + domain + "; path=/;";
  document.cookie = "googtrans=; expires=Thu, 01 Jan 1970 00:00:00 UTC; domain=." + domain + "; path=/;";

  // Se la lingua non è italiano, imposta il cookie con il nuovo codice
  if (lang !== 'it') {
...
window.cambiaLingua = function(codiceLingua, flagEmoji) {
  ...
  localStorage.setItem('flag_selezionata', flagEmoji);

  // Aggiorna l'icona visibile
  const activeFlag = document.getElementById('activeFlag');
  ...
  // Applica le impostazioni del cookie e ricarica la pagina in modo pulito
